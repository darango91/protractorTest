xdescribe('Test with non-angular page', function ( ) {
    it('should search it´s easy', function () {
        browser.ignoreSynchronization = true;
        browser.get('https://www.google.com');

        var search = $('#lst-ib'); //google search bar

        search.sendKeys("It's that easy");
        search.sendKeys(protractor.Key.ENTER);

        expect(search.getAttribute('value')).toEqual("It's that easy");
        browser.sleep(5000);
    });
});
